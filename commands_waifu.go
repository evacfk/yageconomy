...
var (
	WaifuCmdTop = &commands.YAGCommand{
		...
		RunFunc: func(parsed *dcmd.Data) (interface{}, error) {
			...
			return paginatedmessages.CreatePaginatedMessage(parsed.GuildData.GS.ID, parsed.GuildData.CS.ID, parsed.Args[0].Int(), 0, func(p *paginatedmessages.PaginatedMessage, newPage int) (*discordgo.MessageEmbed, error) {
	...
	WaifuCmdAffinity = &commands.YAGCommand{
		...
		RunFunc: func(parsed *dcmd.Data) (interface{}, error) {
			...
			// check cooldown
			var cdResp string
			err := common.RedisPool.Do(retryableredis.Cmd(&cdResp, "SET", fmt.Sprintf("economy_affinity_cd:%d:%d", parsed.GuildData.GS.ID, u.ID), "1", "EX", "1800", "NX"))
			if err != nil {
				return nil, err
			}
	...
	*/
	...
	WaifuShopAdd = &commands.YAGCommand{
		...
		ArgSwitches: []*dcmd.ArgDef{
			&dcmd.ArgDef{Name: "boot", Type: dcmd.Int},
		},
		RunFunc: func(parsed *dcmd.Data) (interface{}, error) {
...
func ListWaifuItems(guildID, channelID int64, u *discordgo.User, currentMoney int64, currencySymbol string) (*discordgo.MessageEmbed, error) {
	...
	return paginatedmessages.NewPaginatedResponse(guildID, channelID, 1, 0, func(p *paginatedmessages.PaginatedMessage, newPage int) (*discordgo.MessageEmbed, error) {
